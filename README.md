...
- 抽幀：每 N 張刪除 1 張
- 批次改名：以命名樣板產生檔名，`[###]` 為編號欄位、`#` 的數量即補零位數，
  編號固定從 0 起算，例如樣板 `F_[#]` 會把 `A.png, B.png, C.png` 改成 `F_0.png, F_1.png, F_2.png`
- 批次改名可勾選「沿用編號」，取用原檔名最後一組連續數字而不重新編號，
  例如 `frame_0037_final.png` 搭配樣板 `Symbol_[#]` 會改成 `Symbol_0037_final.png`；
  找不到編號的檔案標記為不處理，不影響其他檔案
- 批次縮放：支援倍率、絕對尺寸、等比縮放、置入指定範圍；若指定輸出資料夾與來源相同，會自動建立命名子資料夾輸出
- 批次降噪：支援保留細節 / 標準 / 強力三種模式，可產生局部預覽比較效果；固定覆寫原檔
- 抽幀、改名與縮放可在執行時選擇指定資料夾輸出
- 縮放輸出可選擇覆寫原檔或輸出到執行時指定的資料夾
- 所有操作都必須先預覽再執行
- 預覽表格可勾選要納入執行的檔案，錯誤列會停用並以顏色標示
- 抽幀刪除預設移到回收桶
- 改名使用暫存檔名中轉，降低撞名風險
- 勾選包含子資料夾時，每個資料夾會各自重新計數
- 啟動後會背景檢查 GitHub 最新發布，有新版時以頂端橫幅提示下載
- 視窗標題顯示目前的應用程式版本號，例如「影格整理工具 v2.0.0」
...
從 [Releases](https://github.com/WenrongNexus/FrameFileTool/releases) 下載最新版的 zip，
