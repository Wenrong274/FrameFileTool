...
- [x] [正常路徑] 拖放匯入圖片後執行批次縮放並選擇指定資料夾輸出，程式不閃退且正常完成縮放。
- [x] [正常路徑] 以「瀏覽資料夾」選擇來源時，來源與目標相同仍會自動改導到命名子資料夾。
- [x] [邊界] 拖放匯入的檔案在縮放執行完成後仍留在清單中，不被重新掃描清空。
- [x] [錯誤狀態] 刻意觸發未預期例外時，顯示含例外型別與訊息的對話框，關閉後程式仍可繼續操作。

實作結果：

- `OutputFolderResolver.ResolveForResize` 在來源或目標為空白時直接回傳使用者選擇的資料夾，
  不再對空字串做路徑正規化；空來源不可能與目標相同，因此不影響既有的同資料夾自動改導。
- `MainViewModel` 的 `RescanKeepingExclusions` 加上來源資料夾判斷，
  修正拖放匯入的檔案清單在縮放執行後被空路徑重掃清空的問題。
- `App` 註冊 `DispatcherUnhandledException`，顯示例外型別與訊息後標記為已處理，
  UI thread 的未處理例外不再讓程式無聲終止，使用者可回報具體錯誤內容。
- 測試涵蓋來源為空字串與全空白兩種輸入，以及無來源資料夾時不得呼叫掃描服務。

已知限制：

- 全域例外攔截只涵蓋 UI thread；背景執行緒的未觀察例外仍不在處理範圍。
- 錯誤對話框只顯示例外型別與訊息，不寫入檔案 log，回報時需由使用者自行截圖或轉述。
- Magick.NET 原生層的記憶體不足等 process 級失敗仍無法被攔截。
...
- [x] [正常路徑] 啟動程式後，視窗標題顯示「影格整理工具 v2.0.0」。
- [x] [正常路徑] 本機 build 的版本與最後一個已發布 tag 一致，啟動後不再誤跳更新橫幅。
- [x] [邊界] Release workflow 產出的發布包版本仍由 git tag 決定，不受 `Directory.Build.props` 影響。

實作結果：

- `MainViewModel.AppVersionText` 讀取 assembly version 並格式化為 `vX.Y.Z`，
  與 `GitHubUpdateService` 使用同一個版本來源，顯示值與更新比對值不會不同步。
- `MainWindow` 標題改為繫結，顯示為「影格整理工具 vX.Y.Z」；
  版本資訊沿用 v1.7.0 起改由 Windows 原生標題列承載產品識別的方向，不另加自製頁首。
- `Directory.Build.props` 新增 `<Version>` 作為本機 build 的版本基準；
  Release workflow 的 `/p:Version` 是 CLI global property，仍優先於此設定，
  發布包版本依舊由 git tag 決定。
- 原規劃範圍只有標題顯示，實作時發現本機 build 一律為 `1.0.0` 會讓更新橫幅永遠誤跳，
  因此追加版本基準與發布前檢查清單項目。

已知限制：

- `Directory.Build.props` 的 `<Version>` 需在每次發布時手動同步，
  未導入 MinVer 或 Nerdbank.GitVersioning 這類自 tag 推導版本的工具。
- 標題只顯示三段語意版本，不含 build metadata 或 commit hash。
