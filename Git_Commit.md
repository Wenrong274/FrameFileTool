...
- **Header**：必填，包含 type、選填 scope、必填 subject
- **Body**：選填，說明 what 與 why
- **Footer**：選填，標記 breaking change 或關閉 issue
...
3. 標題**首字大寫**
4. 標題結尾**不加句號**
5. 標題使用**祈使現在式**（`Add` 非 `Added`、`Fix` 非 `Fixed`）
6. Body 每行不超過 **72 字元**
7. Body 說明 **what** 與 **why**，而非 how
...
- **BREAKING CHANGE**：標記不向下相容的變動，說明影響範圍
...
- ❌ 不在標題中使用過去式（`Added`、`Fixed`）
- ❌ 不提交未說明原因的大型 commit
- ❌ 不在單一 commit 混合多個無關變更
- ❌ 不提交 build output（`bin/`、`obj/`、`.vs/`）
